...
OSQPProblem::OSQPProblem(const size_t num_of_knots, const double delta_s,
  ...
  ddx_bounds_.resize(num_of_knots_,std::make_pair(-kMaxVarRange, kMaxVarRange));
  pitch_acc_.resize(num_of_knots_,0.0);
}
...
void OSQPProblem::CalculateKernel(std::vector<c_float>* P_data,
  ...
  const int num_of_nonzeros = num_of_variables + n - 2;
  ...
  for (int i = n; i < 2* n; ++i) {
    columns[i].emplace_back(i, (weight_dx_) / (scale_factor_[1] * scale_factor_[1]));
    ++value_index;
  }
    ...
    std::vector<c_float>* upper_bounds)
{
  ...
  const int n = static_cast<int>(num_of_knots_);
  const int num_of_variables = 3 * n;
  const int num_of_constraints = num_of_variables + 2*(n - 1)+ 3;
  ...
  // x(i+1) - x(i) - 2.0 * delta_s * x(i)' = 0
  for (int i = 0; i + 1 < n; ++i)
  {
    variables[i].emplace_back(constraint_index, -1.0/(2*delta_s_));
    variables[i + 1].emplace_back(constraint_index, 1.0/(2*delta_s_));
    variables[n + i].emplace_back(constraint_index, -1.0);
    lower_bounds->at(constraint_index) = pitch_acc_[i];
    upper_bounds->at(constraint_index) = pitch_acc_[i];
    ++constraint_index;
  }
  ...
  for (int i = 0; i + 1 < n; ++i) {
    ...
    variables[2*n + i].emplace_back(constraint_index, -delta_s_ * scale_factor_[1]);
...
bool OSQPProblem::Optimize(const int max_iter) {
  ...
  if (status != 0 && osqp_work == nullptr) {
    std::cout << "failed optimization status:\t" << osqp_work->info->status << std::endl;
...
void OSQPProblem::set_dx_bounds_set(const std::vector<std::pair<double, double>> &acc_bounds_set) {
    dx_bounds_.clear();
    dx_bounds_ = acc_bounds_set;
}

void OSQPProblem::set_pitch_set(const std::vector<double> &refined_pitch)
{
  pitch_acc_ = refined_pitch;
}
